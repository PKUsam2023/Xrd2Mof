...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.atom_latent_emb = nn.Linear(hidden_dim + latent_dim + feature_dim, hidden_dim)
        if act_fn == 'silu':
            self.act_fn = nn.SiLU()
        if dis_emb == 'sin':
    ...
    def forward(self, feature_embedding, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        if self.smooth:
            node_features = self.node_embedding(atom_types.float())
        else:
            node_features = self.node_embedding(atom_types)

        feature_per_atom = feature_embedding.repeat_interleave(num_atoms, dim=0)
        t_per_atom = t.repeat_interleave(num_atoms, dim=0)
        node_features = torch.cat([node_features, feature_per_atom, t_per_atom], dim=1)
