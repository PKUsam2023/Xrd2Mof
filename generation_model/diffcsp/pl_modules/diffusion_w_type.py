...
logging.basicConfig(
    level=logging.INFO,  # Set log level to INFO (outputs INFO, WARNING, ERROR, etc.)
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',  # Output format: time, module name, level, message
    datefmt='%Y-%m-%d %H:%M:%S'  # Time format
)

logger = logging.getLogger(__name__)  # Get a logger; __name__ is the current module name
